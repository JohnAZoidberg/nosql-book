...
1. On what address to listen for oncoming TPC connections. That's what \mintinline{yaml}{listen_address} is for. This is the local/private address.
2. What address to tell other nodes it's reachable under. This is set by \mintinline{yaml}{broadcast_address} and it is the public address that all other nodes must be able to reach.
Therefore in a local network (with no address translation between the nodes) \mintinline{yaml}{listen_address} and \mintinline{yaml}{broadcast_address} are set to the same value.
