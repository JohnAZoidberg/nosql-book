...
Nodes (connected instances running Cassandra) can be hot-swapped or be temporarily down without affecting the availability of the Cassandra cluster.
This is called elastic scalability <- TODO: No it's not! yeah, this kinda is->. Adding a new node and removing one will elasticly increase or decrease
...
Depending on the settings, a cluster of Cassandra nodes can be more or less consistent.
From eventually consistent, the lowest setting for consistency, to highly consistent.
This can be referred to as tunable consistency and allows for a configuration dependent classification in
the CAP Therorem. More on that in section \ref{sec:CassandraClusterArchitecture}.
