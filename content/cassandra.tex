...
Cassandra is a Wide Column Store Database. It is table-like but not relational. The architecture and design make it a distributed and masterless data store for large data. It is written in Java and can be run distributed on commodity hardware. Since it is masterless there is no single point of failure. Nodes (connected instances running Cassandra) can be hot-swapped or be temporarily down without affecting the availability of the Cassandra cluster. Adding a new node and removing one will elasticly increase or decrease the cluster size without any major imapct on the distributed system. This can be reffered to as elastic scalability. All these features make Cassandra highly availabile and fault tolerance. Depending on the settings, a cluster of Cassandra nodes can be more or less consistent. From eventually consistent, the lowest setting for consistency, to highly consistent. This can be referred to as tunable consistency and allows for a configuration dependent classification in the CAP Therorem. More on that in section \ref{sec:CassandraClusterArchitecture}.
...
The following paragraph will describe limits of Cassandra. If the use-case needs any of those features, Cassandra will most likely not suffice to fullfill requirements.
...
Euqually important is the way data has to be stored and accessed. Due to its architecture, data has to be modeled different in Cassandra. ACID transactions are not possible and tables are fine-tuned for pre defined queries. Those querries should be known early on and not change during use. It is not easily possible to change or extent those querries. Furthermore there are no relations beween tables. It is possible to link, connect and reuse IDs but this has to be handled on client side, there are no operations on those IDs available. Additionaly column aggregation operations such as GROUP BY are also not possible, since a wide column oriented storage does not allow such operations.

Having a lot of updates and deletes interspersed with reads slow the system down due to the way of handling requests - mostly just appending data and not changeing records. This has affects on data validation as well. It is not possible to check on write for data uniqueness, constraints or auto increments. 
...
Compared to other database systems like traditional relational database systems, data modelling is different in Cassandra. Here is a little example to understand they different way of thinking when modelling data for Cassandras wide column data store. \\

\ref{fig:cassandra:model_data0} shows an example enity relationship diagramm of data in the context a hotel environment. The hotel has the attributes address, a phone, name and an id as unique primary key. Each hotel has certain amounts of rooms which store information themself and are connected to other entities.
...
\ref{fig:cassandra:model_data2} show this circumstance; queries the system has to answer later on. This diagramm describes a classical use-case for the hotel environment in which the database would come to work. A user searches for hotels, checks information about it and reserves it.
...
Transitioning to the collection of queries in \ref{fig:cassandra:model_data2} to accessible tables can be seen in \ref{fig:cassandra:model_data3}.
...
TODO talk about consistent hashing
% TODO: Cite a consistent hashing paper
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \node[circle,fill=blue] at (180:2.5) {A};
    \node[circle,fill=blue] at (0:2.5) {B};
    \node[circle,fill=blue] at (-20:2.5) {C};
    \node[circle,fill=blue] at (200:2.5) {D};
